    ...
    private void GetAllItemInChance()
    {
        if (itemsClassesToTrade.Contains(ItemClass.Food))
        {
            for (int i = 0; i < gameManager.Food.Count; i++)
                if(gameManager.Food[i].GetComponent<ItemInfo>().chanceOfDrop >= chance - gameManager.luck) ItemsInChance.Add(gameManager.Food[i]);
        }
        if (itemsClassesToTrade.Contains(ItemClass.MelleRangeWeapon))
        {
            for (int i = 0; i < gameManager.MelleRange.Count; i++)
                if (gameManager.MelleRange[i].GetComponent<ItemInfo>().chanceOfDrop >= chance - gameManager.luck) ItemsInChance.Add(gameManager.MelleRange[i]);
        }
        if (itemsClassesToTrade.Contains(ItemClass.ActiveItem))
        {
            for (int i = 0; i < gameManager.ActiveItems.Count; i++)
                if (gameManager.ActiveItems[i].GetComponent<ItemInfo>().chanceOfDrop >= chance - gameManager.luck) ItemsInChance.Add(gameManager.ActiveItems[i]);
        }
        if (itemsClassesToTrade.Contains(ItemClass.PassiveItem))
        {
            for (int i = 0; i < gameManager.PassiveItems.Count; i++)
                if (gameManager.PassiveItems[i].GetComponent<ItemInfo>().chanceOfDrop >= chance - gameManager.luck) ItemsInChance.Add(gameManager.PassiveItems[i]);
        }
    }
